...
let endSpan = (ctx, span, requestSpan) => null
try{
  ...
  console.log('error loading tracing API')
}
  ...
  handler.Entitlements = handler.entitlements = {
    ...
    createMethod: 'createEntitlement',
  ...
  if(process.env.ENABLE_REQUEST_LOGS === 'true'){
    app.use(async (ctx, next) => {
      ...
      if(ctx.url !== '/request-logs'){
        requestLogs.push({ request, response });
      }
    });
  }
  ...
  app.use(async (ctx, next) => {
    ...
    // remove spaces in userName and ID
    if(ctx.request.body?.userName){
      ctx.request.body.userName = ctx.request.body.userName.trim()?.replaceAll(" ", "_")
    }
    if(ctx.request.body?.id){
      ctx.request.body.id = ctx.request.body.id.trim()?.replaceAll(" ", "_")
    }

    console.log(`${method} - ${ctx.request.path}`)
  ...
  //           getGroup (unique)
  // ==========================================
  router.get([`/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`,
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`], async (ctx) => {
  ...
  //           getGroups
  // ==========================================
  router.get(['/(|scim/)(Users|Groups|servicePlans|Entitlements)',
    '/:baseEntity/(|scim/)(Users|Groups|servicePlans|Entitlements)'], async (ctx) => {
      return await TelemetryWrapper(api, async () => {
        requestSpan = verifyTracing(ctx,  'Request')
  ...
  //
  router.post([`/(|scim/)(!${undefined}|Users|Groups|Entitlements)(|.json)(|.xml)`,
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|Entitlements)(|.json)(|.xml)`], async (ctx) => {
    return await TelemetryWrapper(api, async () => {
      requestSpan = verifyTracing(ctx,  'Request')
  ...
  //
  router.delete([`/(|scim/)(!${undefined}|Users|Groups|Entitlements)/:id`,
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|Entitlements)/:id`], async (ctx) => {
    return await TelemetryWrapper(api, async () => {
      requestSpan = verifyTracing(ctx,  'Request')
  ...
  //
  router.patch([`/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`,
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`], async (ctx) => {
    return await TelemetryWrapper(api, async () => {
      requestSpan = verifyTracing(ctx,  'Request')
    if (ctx.query.attributes) ctx.query.attributes = ctx.query.attributes.split(',').map(item => item.trim()).join()
  ...
  //          REPLACE GROUP
  // ==========================================
  router.put([`/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`,
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans|Entitlements)/:id`], async (ctx) => {
    return await TelemetryWrapper(api, async () => {
      requestSpan = verifyTracing(ctx,  'Request')
...
ScimGateway.prototype.formatError = (action, err) => {
  throw  new Error(`${action} error: ${err.response?.data?.message ||err.response?.data?.detail || err.response?.data?.error || err.message}`);
}
...
ScimGateway.prototype.processExtConfig = processExtConfig
